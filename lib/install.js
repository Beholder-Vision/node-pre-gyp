...
function pull_from_s3_api(targetDir, opts, callback) {

  log.info('install', 'Detecting s3 credentials');
  const config = {};
  ...
  log.info('install', 'Authenticating with s3');
  log.info('install', config);
  log.info('install', s3_opts);

  const dataStream = s3.getObject(s3_opts)
    .on('error', function(err) {
      log.info('install', 's3 getObject error: "' + err + '"');
      return callback(err);
    }).createReadStream();
  ...
  if (should_do_source_build) {
    ...
    existsAsync(binary_module, (found) => {
      ...
      makeDir(to).then(() => {
        ...
        } else if (opts.s3Private) {
          pull_from_s3_api(to, opts, after_place);
        } else {
